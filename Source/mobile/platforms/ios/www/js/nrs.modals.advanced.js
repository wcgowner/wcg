...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumWCG",
			addonText: "WCG",
			addonI18n: "wcg_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_wcg', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceWCG';
		context['addonText'] = 'WCG';
		context['addonI18n'] = 'wcg_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_wcg', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
