...
jar/exe/dmg package, or run "java -jar wcg.jar" to start the
...
desktop icons to start the Wcg server.


Using the wcg.zip package:

Unpack the wcg.zip package and open a shell in the resulting wcg
...
the wcg_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.wcg for
configuration files, logs, and the wcg_db database. Unlike run.sh, start.sh
...
message "Wcg server 1.x.x started successfully" in the console log. If run in
...
where the Wcg UI should now be available.
...
to the Wcg installation directory, as the use of special characters may result
...
conf/wcg-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
wcg-default.properties directly, but create a new conf/wcg.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from wcg-default.properties, the
settings in wcg.properties override those in wcg-default.properties. This way,
when upgrading the software, you can safely overwrite wcg-default.properties
with the updated file from the new package, while your customizations remain
safe in the wcg.properties file.


How to contribute?

There are many ways to contribute to Wcg. Here are some examples:
...
The Wcg software is a client-server application. It consists of a java server
...
The java process communicates with peers on port 2052 tcp by default. If you are
...
The user interface is available on port 2082. This port also accepts http API
requests which other Wcg client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
wcg_db directory. When upgrading, you should not delete the old wcg_db
...
wcg_db, except that it will take some extra time to download the blockchain
from scratch.

The default Wcg client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file wcg.log, which gets
overwritten at restart. In case of an error, the wcg.log file may contain
...
wcg.jar file using the enclosed jar.sh script, and then wcg.jar should be
