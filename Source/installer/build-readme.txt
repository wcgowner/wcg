Building the WCG installer

Note: all pathes are relative to the git repository folder (wcg-private)
...
Test X11 installer: java -jar wcg-client-<version>.jar
Test console installer: java -jar wcg-client-<version>.jar -console
...
Copy the content of Java JRE folder (C:\Program Files\Java\jre<version>) into the jre folder under your wcg-private git repository preserving the folder structure
...
Test the installer using Java: java -jar wcg-client-<version>.jar
Test the windows executable: wcg-client-<version>.exe
...
wcg.jar is now self executing, the full classpath is embedded into the jar file manifest automatically by jar.sh
