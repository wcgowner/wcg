...
var NRS = (function(NRS, $) {
    ...
    var coinToPair = function (op, coin) {
        return (op == "buy") ? "WCG_" + coin : coin + "_WCG";
    };

    var pairToCoin = function (pair) {
        if (pair.indexOf("WCG_") == 0) {
            return pair.substring("WCG_".length);
        }
        if (pair.indexOf("_WCG") == pair.length - "_WCG".length) {
            return pair.substring(0, pair.indexOf("_WCG"));
        }
    ...
    var renderExchangeTable = function (op) {
        ...
        async.series(tasks, function (err, results) {
            var table = $("#p_shape_shift_" + op + "_wcg");
            if (err) {
        ...
        apiCall('recenttx/50', {}, 'GET', function (data) {
            ...
            if (data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (String(transaction.curIn).escapeHTML() != "WCG" && String(transaction.curOut).escapeHTML() != "WCG") {
                        continue;
                    }
                    rows += "<tr>";
    ...
    function renderWcgLimit() {
        apiCall('limit/wcg_btc', {}, 'GET', function (data) {
            NRS.logConsole("limit1 " + data.limit);
            if (data.limit) {
                $('#shape_shift_status').html('ok');
                $('#shape_shift_wcg_avail').html(String(data.limit).escapeHTML());
            }
        });
    }
        ...
        apiCall('getcoins', {}, 'GET', function (data) {
            ...
            for (var i = 0; i < inputFields.length; i++) {
                ...
                $.each(data, function (code, coin) {
                    if (code != 'WCG' && coin['status'] == 'available') {
    ...
    NRS.pages.exchange = function() {
        ...
        //$("#exchange_deposit_mail").html($.t("exchange_deposit_mail", { coin: "BTC", agentEmail: "wcg_agent@email.com" }));
        loadDepositCoins();
    ...
    NRS.getFundAccountLink = function() {
        //return "<div class='callout callout-danger'>" +
            ...
            //"data-pair='BTC_WCG'>" + $.t("fund_account_message") + "</a>";
    ...
    $("#m_shape_shift_buy_modal").on("show.bs.modal", function (e) {
        ...
        $("#m_shape_shift_buy_title").html($.t("exchange_wcg_to_coin_shift", { coin: coin }));
        $("#m_shape_shift_buy_min").val(invoker.data("min"));
        $("#m_shape_shift_buy_min_coin").html("WCG");
        $("#m_shape_shift_buy_max").val(invoker.data("max"));
        $("#m_shape_shift_buy_max_coin").html("WCG");
        $("#m_shape_shift_buy_rate").val(invoker.data("rate"));
        $("#m_shape_shift_buy_rate_text").html("WCG/" + coin);
    ...
    $("#m_send_amount_buy_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_buy_title").html($.t("exchange_wcg_to_coin_send_amount", { coin: coin }));
        $("#m_send_amount_buy_withdrawal_amount_coin").html(coin);
        $("#m_send_amount_buy_rate_text").html("WCG/" + coin);
        ...
        apiCall('sendamount', {
            ...
            try {
                ...
                if (amount != data.success.withdrawalAmount) {
                    NRS.showModalError("amount returned from WCG exchange " + data.success.withdrawalAmount +
                    " differs from requested amount " + amount, modal);
                    return;
                }
                if (withdrawal != data.success.withdrawal) {
                    NRS.showModalError("withdrawal address returned from WCG exchange " + data.success.withdrawal +
                ...
                // add 1 WCG fee to make sure the net amount is what requested by shape shift
    ...
    $("#m_shape_shift_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_shape_shift_sell_title").html($.t("exchange_coin_to_wcg_shift", { coin: coin }));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                $("#m_shape_shift_sell_rate_text").html(coin + "/WCG");
                $("#m_shape_shift_sell_fee").val(data.fee);
                $("#m_shape_shift_sell_fee_coin").html("WCG");
    ...
    $("#m_send_amount_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_sell_title").html($.t("exchange_coin_to_wcg_send_amount", { coin: coin }));
        $("#m_send_amount_sell_rate_text").html("WCG/" + coin);
        $("#m_send_amount_sell_fee_coin").html("WCG");
        $("#m_send_amount_sell_withdrawal_amount_coin").html("WCG");
