...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.forms.sellAlias = function ($modal) {
        ...
        if (data.modal == "cancel_alias_sale") {
            ...
            if (data.recipient == NRS.constants.GENESIS_RS) {
                if (!data.priceWCG || data.priceWCG == "0") {
                    return {
                        "error": $.t("error_not_specified", {
        ...
        if (data.priceNQT == "0") {
            ...
            $.growl(
                $.t("success_alias_sell", {
                    ...
                    "price": NRS.convertToWCG(data.priceNQT).escapeHTML()
                }), {
                    "type": "success"
                }
            );
        ...
        NRS.sendRequest("getAlias", {
            ...
            if (response.errorCode) {
                ...
                if (!("priceNQT" in response)) {
                    ...
                    $modal.find("input[name=amountWCG]").val(NRS.convertToWCG(response.priceNQT)).prop("readonly", true);
                }
            }
        }, { isAsync: false });
    });
    ...
    NRS.forms.buyAliasError = function () {
        $("#buy_alias_modal").find("input[name=priceWCG]").prop("readonly", false);
    };
    ...
    NRS.forms.buyAliasComplete = function (response, data) {
        ...
        $.growl(
            $.t("success_alias_buy", {
                "alias_name": data.aliasName,
                "price": NRS.convertToWCG(data.amountNQT).escapeHTML()
            }), {
                "type": "success"
            }
        );
        ...
        if (alias) {
            ...
            NRS.sendRequest("getAlias", {
                ...
                if (response.errorCode) {
                    ...
                    } else if ((aliasURI = /acct:(.*)@wcg/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
    ...
    NRS.forms.setAlias = function ($modal) {
        ...
        if (data.type == "account") {
            if (!(/acct:(.*)@wcg/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
                if (/^(WCG\-)/i.test(data.aliasURI)) {
                    ...
                    if (!address.set(data.aliasURI)) {
                        ...
                        data.aliasURI = "acct:" + data.aliasURI + "@wcg";
                    }
    ...
    function setAliasType(type, uri) {
        ...
        if (type == "uri") {
            ...
            registerAliasUri.val("").mask("WCG-****-****-****-*****");

            if (uri) {
                var match = uri.match(/acct:(.*)@wcg/i);
                if (!match) {
                ...
                } else if (!/^WCG\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
                    uri = NRS.accountRS;
                }
                    ...
                    if ("priceNQT" in response) {
                        if (response.buyer == NRS.account) {
                            message = $.t("alias_sale_direct_offer", {
                                "wcg": NRS.formatAmount(response.priceNQT)
