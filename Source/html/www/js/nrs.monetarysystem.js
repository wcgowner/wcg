...
var NRS = (function (NRS, $, undefined) {
    ...
    search.on("submit", function (e, data) {
        ...
        $("#buy_currency_with_wcg").html($.t("buy_currency_param", { currency: currencyCode}));
        $("#buy_currency_offers").html($.t("offers_to_buy_currency_param", { currency: currencyCode}));
        $("#sell_currency_with_wcg").html($.t("sell_currency_param", { currency: currencyCode}));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
                    $("#ms_your_wcg_balance").html("0");
                } else {
                    $("#ms_your_wcg_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
                }
                NRS.pageLoaded();
                callback(null);
            }
        ...
        NRS.sendRequest("getCurrencyFounders", {
            ...
            if (response.founders && response.founders.length) {
                ...
                for (i = 0; i < response.founders.length; i++) {
                    ...
                    "<td>" + NRS.convertToWCG(amountPerUnitNQT) + "</td>" +
                    "<td>" + NRS.convertToWCG(amountPerUnitNQT.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
            ...
            rows += "<tr>" +
            "<td><b>Totals</b></td>" +
            "<td>" + NRS.convertToWCG(totalAmountReserved) + "</td>" +
            "<td>" + NRS.convertToWCG(totalAmountReserved.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
    ...
    $("#currency_order_modal").on("show.bs.modal", function (e) {
        ...
        var totalWCG = NRS.formatAmount(totalNQT, false, true);
        ...
        $("#currency_order_total").html(totalWCG + " WCG");
    ...
    unitFields.on("change", function() {
        ...
        NRS.sendRequest("getAvailableTo" + NRS.initialCaps(orderType), {
            ...
            rateField.val(rate);
            var amount = NRS.convertToWCG(response.amountNQT);
    ...
    $("#reserve_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency+", {
            ...
            if (response && !response.errorDescription) {
                ...
                $("#reserve_currency_minReserve_text").html(NRS.formatQuantity(NRS.convertToWCG(minReservePerUnitNQT.multiply(new BigInteger(resSupply))), decimals));
                ...
                $("#reserve_currency_currentReserve_text").html(NRS.formatQuantity(NRS.convertToWCG(currentReservePerUnitNQT.multiply(new BigInteger(resSupply))), decimals));
    ...
    reserveCurrencyAmount.blur(function () {
        ...
        var roundUnitAmountNQT = NRS.convertToNQT(NRS.amountToPrecision(NRS.convertToWCG(unitAmountNQT), decimals));
        $("#reserve_currency_total").val(NRS.convertToWCG(roundUnitAmountNQT));
        reserveCurrencyAmount.val(NRS.convertToWCG(new BigInteger(roundUnitAmountNQT).multiply(new BigInteger(resSupply)).toString()));
    });
    ...
    $("#claim_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency", {
            ...
            $("#claimRate").html(NRS.formatAmount(currentReservePerUnitNQT) + " [WCG/" + currencyCode + "]");
        });
