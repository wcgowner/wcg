            ...
            firstNonMaskPos = null, $.each(mask.split(""), function(i, c) {
                ...
                function androidInputEvent() {
                    ...
                    if (oldVal && oldVal.length && oldVal.length > curVal.length) {
                        ...
                        var addressStart = curValUpper.indexOf('WCG-', 4);
                        if (addressStart > 0) {
                            var insertedAddress = curValUpper.substr(addressStart, 24);
                            if (/WCG\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/.test(insertedAddress)) {
                ...
                function keydownEvent(e) {
                    ...
                    if (e.keyCode == 8) {
                        ...
                        if (settings.unmask !== false) {
                            //backspace, remove
                            if ((pos.begin == 0 && pos.end == 24) || (currentInput == "WCG-____-____-____-_____" && pos.begin == 4)) {
                                input.val("");
                                $(this).trigger("unmask");
                                return;
                            }
                ...
                input.bind("keyup.remask", function(e) {
                    if (input.val().toLowerCase() == "wcg-") {
                        input.val("").mask("WCG-****-****-****-*****")./*unbind(".remask").*/trigger("focus");
                    }
                }).bind("paste.remask", function(e) {
                    setTimeout(function() {
                        var newInput = input.val();
                        var pastedData = newInput.substring(4).toUpperCase();
                        if (/WCG\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(pastedData)) {
                            var newAddress = String(pastedData.match("WCG\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}"));
                            input.val(newAddress);
                            checkVal(true);
                        } else if (/^WCG\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(newInput) || /^WCG[A-Z0-9]{17}/i.test(newInput)) {
                            input.mask("WCG-****-****-****-*****").trigger("checkRecipient")/*.unbind(".remask")*/;
                        }
                    }, 0);
                });
