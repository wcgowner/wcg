...
var NRS = (function(NRS, $) {
			...
			if (contacts && contacts.length) {
				...
				$.each(contacts, function (index, contact) {
					...
					rows += "<tr>" +
						...
						"<a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_money_modal' data-contact='" + String(contact.name).escapeHTML() + "'>" + $.t("send_wcg") + "</a>&nbsp;" +
	...
	NRS.forms.addContact = function($modal) {
		...
		if (/^\d+$/.test(data.name) || /^WCG\-/i.test(data.name)) {
			return {
		...
		var address;
		if (/^WCG\-/i.test(data.account_id)) {
			data.account_rs = data.account_id;
			address = new WcgAddress();
	...
	$("#update_contact_modal").on("show.bs.modal", function(e) {
		...
		if (!contactId && NRS.selectedContext) {
			...
			var dbKey = (/^WCG\-/i.test(accountId) ? "accountRS" : "account");
			var dbQuery = {};
