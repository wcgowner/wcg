    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var wcgFields = [
                ...
                ["priceWCG", "priceNQT"],
                ["refundWCG", "refundNQT"],
                ["discountWCG", "discountNQT"],
                ["phasingQuorumWCG", "phasingQuorum"],
                ["phasingMinBalanceWCG", "phasingMinBalance"],
                ["controlQuorumWCG", "controlQuorum"],
                ["controlMinBalanceWCG", "controlMinBalance"],
                ["controlMaxFeesWCG", "controlMaxFees"],
                ["minBalanceWCG", "minBalance"],
                ["shufflingAmountWCG", "amount"],
                ["monitorAmountWCG", "amount"],
                ["monitorThresholdWCG", "threshold"]
            ];

            for (i = 0; i < wcgFields.length; i++) {
                var wcgField = wcgFields[i][0];
                var nqtField = wcgFields[i][1];
                if (wcgField in data) {
                    data[nqtField] = NRS.convertToNQT(data[wcgField]);
                    delete data[wcgField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToWCG(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var accountRS = "";
            if (wcgAddress.set(accountId)) {
                accountRS = wcgAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
