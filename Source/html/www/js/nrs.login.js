...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedWcgAccounts") && NRS.getStrItem("savedWcgAccounts") != ""){
			$('#login_account_container').show();
			$('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedWcgAccounts").split(";");
    ...
    $("#loginButtons").find(".btn").click(function (e) {
        ...
        if (type == "account") {
            ...
            NRS.scanQRCode(readerId, function(text) {
                var wcgAddress = new WcgAddress();
                if (wcgAddress.set(text)) {
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedWcgAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedWcgAccounts');
		} else {
			NRS.setStrItem("savedWcgAccounts", accounts);
		}
		NRS.listAccounts();
	};

    function rememberAccount(account) {
        var accountsStr = NRS.getStrItem("savedWcgAccounts");
        if (!accountsStr) {
            NRS.setStrItem("savedWcgAccounts", account + ";");
            return;
        }
        ...
        NRS.setStrItem("savedWcgAccounts", accountsStr + account + ";");
    }
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					if (NRS.getStrItem("savedWcgAccounts") && NRS.getStrItem("savedWcgAccounts")!=""){
						...
						accounts = NRS.getStrItem("savedWcgAccounts").split(";");
						$.each(accounts, function(index, account) {
							if (account != ''){
	...
	NRS.initPluginWarning = function() {
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your Wcg passphrase.";
			html += "</p>";
			...
			html += "you can loose your WCG! In doubt don't run plugins with accounts ";
			html += "used to store larger amounts of WCG now or in the future.";
			html += "</p>";
			html += "</div>";
