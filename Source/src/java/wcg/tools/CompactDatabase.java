 ...
 *
 *   java -cp "classes:lib/*:conf" wcg.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dwcg.runtime.mode=desktop wcg.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Wcg properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "wcg.testDb" : "wcg.db";
        String dbType = Wcg.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Wcg.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Wcg.getDbDir(Wcg.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Wcg.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Wcg.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Wcg.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "wcg.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "wcg.mv.db");
            if (!dbFile.exists()) {
