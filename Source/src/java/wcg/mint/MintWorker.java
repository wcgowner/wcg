...
import wcg.util.TrustAllSSLProvider;
...
public class MintWorker {
    ...
    private void mint() {
        String currencyCode = Convert.emptyToNull(Wcg.getStringProperty("wcg.mint.currencyCode"));
        if (currencyCode == null) {
            throw new IllegalArgumentException("wcg.mint.currencyCode not specified");
        }
        String secretPhrase = Convert.emptyToNull(Wcg.getStringProperty("wcg.mint.secretPhrase", null, true));
        if (secretPhrase == null) {
            throw new IllegalArgumentException("wcg.mint.secretPhrase not specified");
        }
        boolean isSubmitted = Wcg.getBooleanProperty("wcg.mint.isSubmitted");
        boolean isStopOnError = Wcg.getBooleanProperty("wcg.mint.stopOnError");
        ...
        String unitsStr = Wcg.getStringProperty("wcg.mint.unitsPerMint");
        double wholeUnits = 1;
        ...
        long initialNonce = Wcg.getIntProperty("wcg.mint.initialNonce");
        if (initialNonce == 0) {
            initialNonce = new Random().nextLong();
        }
        int threadPoolSize = Wcg.getIntProperty("wcg.mint.threadPoolSize");
        if (threadPoolSize == 0) {
    ...
    private JSONObject mintImpl(String secretPhrase, long accountId, long units, long currencyId, byte algorithm,
        ...
        if (isSubmitted) {
            ...
            response.put("message", "wcg.mint.isSubmitted=false therefore currency mint transaction is not submitted");
        }
        return response;
    }
    ...
    private JSONObject currencyMint(String secretPhrase, long currencyId, long nonce, long units, long counter) {
        ...
        Transaction.Builder builder = Wcg.newTransactionBuilder(Crypto.getPublicKey(secretPhrase), 0, Constants.ONE_WCG,
    ...
    private JSONObject getJsonResponse(Map<String, String> params) {
        ...
        String host = Convert.emptyToNull(Wcg.getStringProperty("wcg.mint.serverAddress"));
        if (host == null) {
            try {
        ...
        boolean useHttps = Wcg.getBooleanProperty("wcg.mint.useHttps");
        if (useHttps) {
            protocol = "https";
            HttpsURLConnection.setDefaultSSLSocketFactory(TrustAllSSLProvider.getSslSocketFactory());
            HttpsURLConnection.setDefaultHostnameVerifier(TrustAllSSLProvider.getHostNameVerifier());
        }
        int port = Constants.isTestnet ? API.TESTNET_API_PORT : Wcg.getIntProperty("wcg.apiServerPort");
        String urlParams = getUrlParams(params);
        URL url;
        try {
            url = new URL(protocol, host, port, "/wcg?" + urlParams);
