...
/**
 * Handle logging for the Wcg node server
 */
public final class Logger {
     ...
     * The wcg/conf/logging-default.properties and wcg/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "wcg.util.WcgLogManager");
        if (!(LogManager.getLogManager() instanceof WcgLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("wcg.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Wcg.loadProperties(loggingProperties, "logging-default.properties", true);
                Wcg.loadProperties(loggingProperties, "logging.properties", false);
                Wcg.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(wcg.Wcg.class);
        enableStackTraces = Wcg.getBooleanProperty("wcg.enableStackTraces");
        enableLogTraceback = Wcg.getBooleanProperty("wcg.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof WcgLogManager) {
            ((WcgLogManager) LogManager.getLogManager()).wcgShutdown();
        }
    }
