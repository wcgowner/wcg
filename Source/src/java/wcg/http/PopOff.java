...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            Wcg.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = Wcg.getBlockchainProcessor().popOffTo(Wcg.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = Wcg.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            Wcg.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> Wcg.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }
