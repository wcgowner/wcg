...
import wcg.AccountRestrictions;
import wcg.Alias;
import wcg.Appendix;
import wcg.Asset;
import wcg.AssetDelete;
import wcg.AssetDividend;
import wcg.AssetTransfer;
import wcg.Attachment;
import wcg.Block;
import wcg.Constants;
import wcg.Currency;
import wcg.CurrencyExchangeOffer;
import wcg.CurrencyFounder;
import wcg.CurrencyTransfer;
import wcg.CurrencyType;
import wcg.DigitalGoodsStore;
import wcg.Exchange;
import wcg.ExchangeRequest;
import wcg.FundingMonitor;
import wcg.Generator;
import wcg.HoldingType;
import wcg.MonetarySystem;
import wcg.Wcg;
import wcg.Order;
import wcg.PhasingPoll;
import wcg.PhasingVote;
import wcg.Poll;
import wcg.PrunableMessage;
import wcg.Shuffler;
import wcg.Shuffling;
import wcg.ShufflingParticipant;
import wcg.TaggedData;
import wcg.Token;
import wcg.Trade;
import wcg.Transaction;
import wcg.Vote;
import wcg.VoteWeighting;
import wcg.crypto.Crypto;
import wcg.crypto.EncryptedData;
import wcg.db.DbIterator;
import wcg.peer.Hallmark;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Wcg.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWCG", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWCG", account.getEffectiveBalanceWCG(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceWCG", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_WCG));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.WCG) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Wcg.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Wcg.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Wcg.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Wcg.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Wcg.getBlockchain().getTransaction(entry.getEventId());
