...
import wcg.TransactionProcessor;
 ...
 * has not been received within wcg.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by wcg.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Wcg.getIntProperty("wcg.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Wcg.getIntProperty("wcg.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Wcg.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Wcg.getTransactionProcessor();
    ...
    /** Wcg event listeners */
    private final List<WcgEventListener> wcgEventListeners = new ArrayList<>();
     ...
     *
     * Wcg event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Wcg event registrations
     ...
     *
     * @param   eventRegistrations      Wcg event registrations
     * @throws  EventListenerException  Invalid Wcg event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<WcgEventListener> it = wcgEventListeners.iterator();
                while (it.hasNext()) {
                    WcgEventListener listener = it.next();
                ...
                if (addListener) {
                    WcgEventListener listener = new WcgEventListener(event);
                    listener.addListener();
                    wcgEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            wcgEventListeners.forEach(WcgEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Wcg event listener
     */
    private class WcgEventListener {

        /** Event handler */
        private final WcgEventHandler eventHandler;

        /**
         * Create the Wcg event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public WcgEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Wcg listener for this event
         */
        ...
        /**
         * Remove the Wcg listener for this event
         */
        ...
        /**
         * Return the hash code for this Wcg event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Wcg events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof WcgEventListener) &&
                    eventHandler.equals(((WcgEventListener)obj).eventHandler));
        }

        /**
         * Wcg listener event handler
         */
        private abstract class WcgEventHandler {
            ...
            /**
             * Create the Wcg event handler
             *
             * @param   eventRegistration   Event registration
             */
            public WcgEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof WcgEventHandler) &&
                                        owner == ((WcgEventHandler)obj).owner &&
                                        accountId == ((WcgEventHandler)obj).accountId &&
                                        event == ((WcgEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends WcgEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends WcgEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends WcgEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends WcgEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Wcg listener event
        ...
        /**
         * Return the Wcg listener event
         *
         * @return                  Wcg listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
