...
import static wcg.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static wcg.http.JSONResponses.INCORRECT_DEADLINE;
import static wcg.http.JSONResponses.INCORRECT_EC_BLOCK;
import static wcg.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static wcg.http.JSONResponses.INCORRECT_WHITELIST;
import static wcg.http.JSONResponses.MISSING_DEADLINE;
import static wcg.http.JSONResponses.MISSING_SECRET_PHRASE;
import static wcg.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Wcg.getBlockchain().getHeight() + 1,
                Wcg.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws WcgException {
        ...
        //Logger.logMessage("ecBlockId="+ecBlockId+" | ecBlockHeight="+ecBlockHeight+" | Wcg.getBlockchain().getBlockIdAtHeight(ecBlockHeight)="+Wcg.getBlockchain().getBlockIdAtHeight(ecBlockHeight));
        //ecBlockId = new BigInteger("429943628837330866").longValue();
        if (ecBlockId != 0 && ecBlockId != Wcg.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
        	//suse 10/03/2017
        	JSONObject response = new JSONObject();
            response.put("errorCode", 4);
            response.put("errorDescription", "Incorrect ecBlockId="+ecBlockId+" does not match the block id at ecBlockHeight="+ecBlockHeight+" - Wcg.getBlockchain().getBlockIdAtHeight(ecBlockHeight)="+Wcg.getBlockchain().getBlockIdAtHeight(ecBlockHeight));
            return JSON.prepare(response);
        	//JSONStreamAware msg = incorrect("ecBlockId", "ecBlockId does not match the block id at ecBlockHeight");
        	//var message = JSONStreamAware (incorrect"Incorrect ecBlockId="+ecBlockId+" does not match the block id at ecBlockHeight="+ecBlockHeight+" - Wcg.getBlockchain().getBlockIdAtHeight(ecBlockHeight)="+Wcg.getBlockchain().getBlockIdAtHeight(ecBlockHeight); 
        	//return 
            //return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Wcg.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Wcg.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (WcgException.NotYetEncryptedException ignore) {}
        ...
        } catch (WcgException.NotYetEnabledException e) {
