...
import static wcg.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static wcg.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
import static wcg.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
...
public final class API {
    ...
    public static final String adminPassword = Wcg.getStringProperty("wcg.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Wcg.getIntProperty("wcg.maxAPIRecords");
    static final boolean enableAPIUPnP = Wcg.getBooleanProperty("wcg.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Wcg.getIntProperty("wcg.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Wcg.getBooleanProperty("wcg.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(Wcg.getStringListProperty("wcg.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Wcg.getStringListProperty("wcg.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Wcg.getStringListProperty("wcg.allowedBotHosts");
        ...
        boolean enableAPIServer = Wcg.getBooleanProperty("wcg.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Wcg.getIntProperty("wcg.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Wcg.getIntProperty("wcg.apiServerSSLPort");
            final String host = Wcg.getStringProperty("wcg.apiServerHost");
            disableAdminPassword = Wcg.getBooleanProperty("wcg.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Wcg.getBooleanProperty("wcg.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Wcg.getUserHomeDir()).resolve(Paths.get(Wcg.getStringProperty("wcg.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Wcg.getStringProperty("wcg.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Wcg.getStringProperty("wcg.keyStoreType"));
                List<String> ciphers = Wcg.getStringListProperty("wcg.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Wcg.getStringProperty("wcg.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Wcg.getStringProperty("wcg.apiWelcomeFile")});
            }

            String javadocResourceBase = Wcg.getStringProperty("wcg.javadocResourceBase");
            ...
            if (Wcg.getBooleanProperty("wcg.enableApiTestUI")) {
            ...
            if (Wcg.getBooleanProperty("wcg.apiFrameOptionsSameOrigin")) {
