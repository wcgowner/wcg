...
import static wcg.http.JSONResponses.INCORRECT_BLOCK;
import static wcg.http.JSONResponses.INCORRECT_HEIGHT;
import static wcg.http.JSONResponses.INCORRECT_TIMESTAMP;
import static wcg.http.JSONResponses.UNKNOWN_BLOCK;
...
public final class GetBlock extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (blockValue != null) {
            try {
                blockData = Wcg.getBlockchain().getBlock(Convert.parseUnsignedLong(blockValue));
            ...
            try {
                ...
                blockData = Wcg.getBlockchain().getBlockAtHeight(height);
            } catch (RuntimeException e) {
                return INCORRECT_HEIGHT;
            }
            ...
            try {
                ...
                blockData = Wcg.getBlockchain().getLastBlock(timestamp);
            } catch (RuntimeException e) {
                return INCORRECT_TIMESTAMP;
            }
        } else {
            blockData = Wcg.getBlockchain().getLastBlock();
        }
