...
import static wcg.http.JSONResponses.ERROR_DISABLED;
import static wcg.http.JSONResponses.ERROR_INCORRECT_REQUEST;
import static wcg.http.JSONResponses.ERROR_NOT_ALLOWED;
import static wcg.http.JSONResponses.LIGHT_CLIENT_DISABLED_API;
import static wcg.http.JSONResponses.POST_REQUIRED;
import static wcg.http.JSONResponses.REQUIRED_BLOCK_NOT_FOUND;
import static wcg.http.JSONResponses.REQUIRED_LAST_BLOCK_NOT_FOUND;
...
public final class APIServlet extends HttpServlet {
    ...
    public abstract static class APIRequestHandler {
        ...
        protected abstract JSONStreamAware processRequest(HttpServletRequest request) throws WcgException;

        protected JSONStreamAware processRequest(HttpServletRequest request, HttpServletResponse response) throws WcgException {
    ...
    private static final boolean enforcePost = Wcg.getBooleanProperty("wcg.apiServerEnforcePOST");
    ...
    static {
        ...
        API.disabledAPIs.forEach(api -> {
            ...
            if (handler == null) {
                throw new RuntimeException("Invalid API in wcg.disabledAPIs: " + api);
            }
            ...
            try {
                ...
                try {
                    try {
                        ...
                        if (requireBlockId != 0 && !Wcg.getBlockchain().hasBlock(requireBlockId)) {
                            response = REQUIRED_BLOCK_NOT_FOUND;
                            return;
                        }
                        if (requireLastBlockId != 0 && requireLastBlockId != Wcg.getBlockchain().getLastBlock().getId()) {
                        ...
                        if (requireLastBlockId == 0 && requireBlockId != 0 && response instanceof JSONObject) {
                            ((JSONObject) response).put("lastBlock", Wcg.getBlockchain().getLastBlock().getStringId());
                        }
                    } finally {
                        if (apiRequestHandler.startDbTransaction()) {
