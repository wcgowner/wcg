...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Wcg.getBooleanProperty("wcg.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistingPeriod = Wcg.getIntProperty("wcg.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Wcg.getStringProperty("wcg.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Wcg.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, Wcg.getEpochTime() + blacklistingPeriod);
