...
public final class Poll extends AbstractPoll {
    ...
    private static final boolean isPollsProcessing = Wcg.getBooleanProperty("wcg.processPolls");
    ...
    public static DbIterator<Poll> getActivePolls(int from, int to) {
        return pollTable.getManyBy(new DbClause.IntClause("finish_height", DbClause.Op.GT, Wcg.getBlockchain().getHeight()), from, to);
    }
    ...
    public static DbIterator<Poll> getPollsByAccount(long accountId, boolean includeFinished, boolean finishedOnly, int from, int to) {
        ...
        if (finishedOnly) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.LTE, Wcg.getBlockchain().getHeight()));
        } else if (!includeFinished) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.GT, Wcg.getBlockchain().getHeight()));
        }
    ...
    public static DbIterator<Poll> searchPolls(String query, boolean includeFinished, int from, int to) {
        DbClause dbClause = includeFinished ? DbClause.EMPTY_CLAUSE : new DbClause.IntClause("finish_height", DbClause.Op.GT, Wcg.getBlockchain().getHeight());
    ...
    public boolean isFinished() {
        return finishHeight <= Wcg.getBlockchain().getHeight();
    }

    private List<OptionResult> countResults(VoteWeighting voteWeighting) {
        int countHeight = Math.min(finishHeight, Wcg.getBlockchain().getHeight());
        if (countHeight < Wcg.getBlockchainProcessor().getMinRollbackHeight()) {
            return null;
        }
