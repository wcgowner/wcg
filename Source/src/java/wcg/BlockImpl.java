...
final class BlockImpl implements Block {
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws WcgException.NotValidException {
        try {
            ...
            if (!block.checkSignature()) {
                throw new WcgException.NotValidException("Invalid block signature");
            }
            return block;
    ...
    byte[] bytes() {
        if (bytes == null) {
            ...
            if (version < 3) {
                buffer.putInt((int) (totalAmountNQT / Constants.ONE_WCG));
                buffer.putInt((int) (totalFeeNQT / Constants.ONE_WCG));
            } else {
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
        ...
        try {
            ...
            long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceWCG();
    ...
    void apply() {
        ...
        if (this.height > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f " + Constants.COIN_NAME + " to forger at height %d", ((double)backFees[i])/Constants.ONE_WCG, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f " + Constants.COIN_NAME + " at height %d", ((double)totalBackFees)/Constants.ONE_WCG, this.height);
        }
