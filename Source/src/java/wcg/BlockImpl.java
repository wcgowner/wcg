    ...
    void apply() {
        ...
        if (this.height > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f WCG to forger at height %d", ((double)backFees[i])/Constants.ONE_WCG, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f WCG at height %d", ((double)totalBackFees)/Constants.ONE_WCG, this.height);
        }
