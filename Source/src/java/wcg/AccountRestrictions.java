...
import wcg.WcgException.AccountControlException;
...
public final class AccountRestrictions {
    ...
    public static final class PhasingOnly {
        ...
        private void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws AccountControlException {
            if (!validatingAtFinish && maxFees > 0 && Math.addExact(transaction.getFeeNQT(), PhasingPoll.getSenderPhasedTransactionFees(transaction.getSenderId())) > maxFees) {
                throw new AccountControlException(String.format("Maximum total fees limit of %f " + Constants.COIN_NAME + " exceeded", ((double)maxFees)/Constants.ONE_WCG));
            }
            ...
            if (!validatingAtFinish) {
                int duration = phasingAppendix.getFinishHeight() - Wcg.getBlockchain().getHeight();
    ...
    static void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws WcgException.NotCurrentlyValidException {
        Account senderAccount = Account.getAccount(transaction.getSenderId());
        if (senderAccount == null) {
            throw new WcgException.NotCurrentlyValidException("Account " + Long.toUnsignedString(transaction.getSenderId()) + " does not exist yet");
        }
