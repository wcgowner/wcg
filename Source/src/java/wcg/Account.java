...
import wcg.db.DbKey;
import wcg.db.DbUtils;
import wcg.db.DerivedDbTable;
import wcg.db.VersionedEntityDbTable;
import wcg.db.VersionedPersistentDbTable;
...
public final class Account {
    ...
    public static final class PublicKey {
        ...
        private void save(Connection con) throws SQLException {
            height = Wcg.getBlockchain().getHeight();
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < Wcg.getBlockchainProcessor().getMinRollbackHeight()) {
                throw new IllegalArgumentException("Historical data as of height " + height +" not available.");
            }
            if (height > Wcg.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + Wcg.getBlockchain().getHeight());
            }
        }
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = Wcg.getBooleanProperty("wcg.enablePublicKeyCache") ?
    ...
    static {
        ...
        if (publicKeyCache != null) {
            ...
            Wcg.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public long getEffectiveBalanceWCG() {
        return getEffectiveBalanceWCG(Wcg.getBlockchain().getHeight());
    }

    public long getEffectiveBalanceWCG(int height) {
        if (height >= Constants.TRANSPARENT_FORGING_BLOCK_6) {
        ...
        if (height < Constants.TRANSPARENT_FORGING_BLOCK_3) {
            ...
            if (Arrays.binarySearch(Genesis.GENESIS_RECIPIENTS, id) >= 0) {
                return balanceNQT / Constants.ONE_WCG;
            }
            long receivedInLastBlock = 0;
            for (Transaction transaction : Wcg.getBlockchain().getBlockAtHeight(height).getTransactions()) {
                if (id == transaction.getRecipientId()) {
                    receivedInLastBlock += transaction.getAmountNQT();
                }
            }
            return (balanceNQT - receivedInLastBlock) / Constants.ONE_WCG;
        }
        ...
        try {
            ...
            return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_NQT) ? 0 : effectiveBalanceNQT / Constants.ONE_WCG;
        } finally {
    ...
    public long getGuaranteedBalanceNQT() {
        return getGuaranteedBalanceNQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, Wcg.getBlockchain().getHeight());
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < Wcg.getBlockchainProcessor().getMinRollbackHeight()
                    || height > Wcg.getBlockchain().getHeight()) {
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = Wcg.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= Wcg.getBlockchain().getHeight() - 1) {
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.WCG_BALANCE, null, feeNQT, this.balanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.WCG_BALANCE, null, amountNQT, this.balanceNQT));
            }
        }
    }
    ...
    void addToUnconfirmedBalanceNQT(LedgerEvent event, long eventId, long amountNQT, long feeNQT) {
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_WCG_BALANCE, null, feeNQT, this.unconfirmedBalanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_WCG_BALANCE, null, amountNQT, this.unconfirmedBalanceNQT));
            }
        }
    }
