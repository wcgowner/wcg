...
cd -
zip -q -X -r ${PACKAGE}.zip wcg -x \*/.idea/\* \*/.gitignore \*/.git/\* \*.iml wcg/conf/wcg.properties wcg/conf/logging.properties wcg/conf/localstorage/\*
rm -rf wcg

echo creating change log ${CHANGELOG}
echo -e "Release $1\n" > ${CHANGELOG}
echo -e "https://bitbucket.org/JeanLucPicard/wcg/downloads/${PACKAGE}.exe\n" >> ${CHANGELOG}
echo -e "sha256:\n" >> ${CHANGELOG}
sha256sum ${PACKAGE}.exe >> ${CHANGELOG}

echo -e "https://bitbucket.org/JeanLucPicard/wcg/downloads/${PACKAGE}.jar\n" >> ${CHANGELOG}
echo -e "sha256:\n" >> ${CHANGELOG}
